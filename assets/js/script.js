...
(function ( ) {
	...
	var userAgent = navigator.userAgent.toLowerCase(),
			...
			plugins = {
                ...
                rdDifferentAddress: $("#ship_to_different_address"),
				regula: $("[data-constraints]"),
				selectFilter: $(".product-top-panel-sorting select"),
				selectInput: $("select"),
				...
				filterPriceButton: $('#filter_button')
			};
	...
	$(function () {
        ...
        function autofillCheckoutForm(form) {
            // Verifică dacă există stocare locală în browser
            if(typeof(Storage) !== "undefined") {
                // Când se face o schimbare în formular, salvează datele
                form.on('change', 'input, select', function(e) {
                    localStorage.setItem(e.target.name, e.target.value);
                });
        
                // La încărcarea paginii, populează formularul cu datele salvate
        ...
        if (plugins.rdDifferentAddress.length) {
            let el = plugins.rdDifferentAddress;
            el.on("change", hideShippingForm);
            function hideShippingForm(){
                let shippingForm = el.closest('form').find('#shipping-form');
                if(el.is(':checked'))
		...
		if (plugins.selectFilter.length) {
			...
			for (i = 0; i < plugins.selectFilter.length; i++) {
                ...
                select.change( function(){
                    window.location.search = '?orderby=' + select.val();
                });
			}
		}

		// Select2
		if (plugins.selectInput.length) {
			var i;
			for (i = 0; i < plugins.selectInput.length; i++) {
				var select = $(plugins.selectInput[i]),
						selectStyle = 'html-' + select.attr('data-style') + '-select';
				$html.addClass(selectStyle);
		...
		// Filter price 
		if (plugins.filterPriceButton.length){
			plugins.filterPriceButton.on( 'click', function(){
				let minPriceValue = $('#min_price_value').val();
				let maxPriceValue = $('#max_price_value').val();

				if ( minPriceValue.length && maxPriceValue.length) {
					window.location.search = `?min_price=${minPriceValue}&max_price=${maxPriceValue}`;
				}
			});
		}

        // Form data save on reload page
        if (plugins.checkoutForm.length){
            autofillCheckoutForm(plugins.checkoutForm);
        }
	});
}());
